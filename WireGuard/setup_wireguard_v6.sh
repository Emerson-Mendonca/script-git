...
# Por padrão, todas as portas são acessíveis apenas via VPN; portas em allowed_ports.txt são abertas externamente
...
setup_firewall() {
    ...
    # Permite todo tráfego TCP/UDP na interface wg0 (VPN-only por padrão)
    sudo iptables -A INPUT -i wg0 -p tcp -j ACCEPT
    sudo iptables -A INPUT -i wg0 -p udp -j ACCEPT
    
    # Permite portas específicas em todas as interfaces (acesso externo)
...
add_port() {
    read -p "Digite a porta para adicionar (acesso externo, 1-65535): " port </dev/tty
    if [[ "$port" =~ ^[0-9]+$ && "$port" -ge 1 && "$port" -le 65535 ]]; then
        if ! grep -Fx "$port" /etc/wireguard/allowed_ports.txt > /dev/null; then
            echo "$port" >> /etc/wireguard/allowed_ports.txt
            reapply_firewall
            print_message "Porta $port adicionada com sucesso e agora acessível externamente."
        else
...
remove_port() {
    read -p "Digite a porta para remover (restringir a VPN, 1-65535): " port </dev/tty
    if [[ "$port" =~ ^[0-9]+$ && "$port" -ge 1 && "$port" -le 65535 ]]; then
        if grep -Fx "$port" /etc/wireguard/allowed_ports.txt > /dev/null; then
            ...
            reapply_firewall
            print_message "Porta $port removida com sucesso e agora restrita a VPN."
        else
...
list_ports() {
    print_message "Portas atualmente abertas para acesso externo:"
    if [ -f /etc/wireguard/allowed_ports.txt ] && [ -s /etc/wireguard/allowed_ports.txt ]; then
        cat /etc/wireguard/allowed_ports.txt | sort -n
    else
        print_warning "Nenhuma porta aberta externamente. Todas as portas são VPN-only."
    fi
}
...
check_final_status() {
    ...
    if command -v docker &> /dev/null && systemctl is-active docker &> /dev/null; then
        print_warning "⚠️ Docker está ativo. Verifique se o gerenciamento de iptables está desativado."
    fi
}
...
show_menu() {
    ...
    echo "=========================================="
    echo "1. Adicionar porta para acesso externo"
    echo "2. Remover porta (restringir a VPN)"
    echo "3. Listar portas com acesso externo"
...
show_summary() {
    ...
    echo ""
    print_message "Por padrão, todas as portas são acessíveis apenas via VPN."
    echo "Para abrir uma porta externamente, adicione-a usando a opção 1."
    echo ""
    print_message "Para gerenciar portas:"
...
EOF
    ...
    while $whileBool; do
        show_menu
        read -p "Selecione uma opção (1-6): " choice </dev/tty
        case $choice in
            1)
                add_port
            ...
            4)
                ...
                ;;
            5)
                print_message "Limpar configurações antigas..." </dev/tty
                cleanup_old_configs
                setup_firewall
                read -p "Pressione Enter para continuar..." </dev/tty
                ;;
